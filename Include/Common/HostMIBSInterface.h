...
typedef union _U_MIBS_IP_ADDRESS
{
    struct 
	{
		...
		B_UINT32		ulIpv4Addr[MIBS_MAX_IP_RANGE_LENGTH]; 
		//Source Ip Mask Address Range
		B_UINT32       ulIpv4Mask[MIBS_MAX_IP_RANGE_LENGTH];
	};
	struct
	{
		//Source Ip Address Range
		B_UINT32		ulIpv6Addr[MIBS_MAX_IP_RANGE_LENGTH * 4]; 
		//Source Ip Mask Address Range
		B_UINT32       ulIpv6Mask[MIBS_MAX_IP_RANGE_LENGTH * 4];
...
typedef struct _S_MIBS_HOST_INFO
{
	...
	B_UINT32			NumDesUsed;
	B_UINT32			CurrNumFreeDesc;
	B_UINT32			PrevNumFreeDesc;
	// to keep track the no of byte recieved
	B_UINT32			PrevNumRcevBytes;
	B_UINT32			CurrNumRcevBytes;

	/* QOS Related */
	B_UINT32			BEBucketSize;
	B_UINT32			rtPSBucketSize;
	B_UINT32			LastTxQueueIndex;
...
typedef struct _S_MIBS_PHS_RULE
{
	...
	B_UINT32	    PHSModifiedNumPackets;
	B_UINT32		PHSErrorNumPackets;
}S_MIBS_PHS_RULE;
...
{
	B_UINT32 cm_responses;
	B_UINT32 cm_control_newdsx_multiclassifier_resp;
	B_UINT32 link_control_resp;
	B_UINT32 status_rsp;
	B_UINT32 stats_pointer_resp;
	B_UINT32 idle_mode_status;
	B_UINT32 auth_ss_host_msg;
	B_UINT32 low_priority_message;
