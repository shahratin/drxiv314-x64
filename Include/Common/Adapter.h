...
{
    B_UINT32 		SrcIpAddressCount;
    B_UINT32 		SrcIpAddress[MAX_CONNECTIONS];
    B_UINT32 		SrcIpMask[MAX_CONNECTIONS];

    B_UINT32 		DestIpAddressCount;
    B_UINT32 		DestIpAddress[MAX_CONNECTIONS];
    B_UINT32 		DestIpMask[MAX_CONNECTIONS];
...
typedef union _U_IP_ADDRESS
{
    struct 
	{
		B_UINT32				ulIpv4Addr[MAX_IP_RANGE_LENGTH];//Source Ip Address Range 
		B_UINT32               ulIpv4Mask[MAX_IP_RANGE_LENGTH];//Source Ip Mask Address Range
	};
	struct
	{
		B_UINT32				ulIpv6Addr[MAX_IP_RANGE_LENGTH * 4];//Source Ip Address Range 
		B_UINT32               ulIpv6Mask[MAX_IP_RANGE_LENGTH * 4];//Source Ip Mask Address Range
...
typedef INT (*FP_FLASH_BULK_WRITE)(struct _MINI_ADAPTER*,PUINT,UINT,UINT,BOOLEAN);
	
typedef INT (*FP_FLASH_BULK_READ)(struct _MINI_ADAPTER*,PUINT,UINT,UINT);
...
//	spinlock_t			RxControlQueuelock;
	...
	B_UINT32		    	bcm_jiffies;	/* Store Jiffies value */
	...
	B_UINT32			    BEBucketSize;
	B_UINT32			    rtPSBucketSize;
	UCHAR			    LinkStatus;
	BOOLEAN			    AutoLinkUp;
	BOOLEAN			    AutoSyncup;
	...
	B_UINT32				ulFreeTargetBufferCnt;
	B_UINT32              	ulCurrentTargetBuffer;
	B_UINT32              	ulTotalTargetBuffersAvailable;
...
//	struct semaphore 	data_packet_queue_lock;
	...
	B_UINT32                   ulFlashControlSectionStart; 
	B_UINT32                   ulFlashWriteSize;
	B_UINT32                   ulFlashID;
	FP_FLASH_WRITE          fpFlashWrite;	
	FP_FLASH_WRITE_STATUS   fpFlashWriteWithStatusCheck;
	B_UINT32			localjiffies;			
...
//	BOOLEAN				InterfaceUpStatus;
	...
	FLASH_SHM_INFO			sFlashShmInfo[MAX_CA_COUNT];
	UINT					uiCAcount;
	FP_FLASH_BULK_WRITE     fpFlashBulkWrite;
	FP_FLASH_BULK_READ		fpFlashBulkRead;
	BOOLEAN			        bWriteWOErase;
	BOOLEAN					bFlashSHMEnabled;
	BOOLEAN                 bDisableFastFlashWrite;
	BOOLEAN                 bUseFlashShmPtr;
	UCHAR	ucDsxLinkUpCfg;
	UCHAR	ucDsxConnBitMap;
	UCHAR	bPatternWritten;
	UINT	uiHiPriorityPattern;
	struct semaphore	PatternWriteLock;
};
