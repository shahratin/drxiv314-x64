	...
	if(ntohl(*puiBuffer) == GO_TO_IDLE_MODE_PAYLOAD)
	{
		...
		if(ntohl(*(puiBuffer+1)) == 0 )
		{
			...
			else if(Adapter->ulPowerSaveMode != DEVICE_POWERSAVE_MODE_AS_PROTOCOL_IDLE_MODE)
			{
				...
				{	
                    BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, IDLE_MODE, DBG_LVL_ALL, "\n\nwriting 0 to register:DEVICE_INT_OUT_EP_REG1 \n\n");
	    			status = wrmalt (Adapter, DEVICE_INT_OUT_EP_REG1, &uiRegRead, sizeof(uiRegRead));
		    		if(status)
			    	{
				    	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0, "wrm failed while clearing H/W Abort	Reg1");
					    return status;
				    }
		   		}
			}	
			...
			Adapter->bPatternWritten = FALSE;
			Adapter->uiHiPriorityPattern = 0;
...
int InterfaceAbortIdlemode(PMINI_ADAPTER Adapter, unsigned int Pattern)
{
	...
	if( (Adapter->ulPowerSaveMode != DEVICE_POWERSAVE_MODE_AS_PROTOCOL_IDLE_MODE) &&
		(Adapter->ulPowerSaveMode != DEVICE_POWERSAVE_MODE_AS_MANUAL_CLOCK_GATING) )
	{
		/*
	...
	if(Adapter->uiHiPriorityPattern == ABORT_IDLE_SYNCDOWN_NO_DREG)
		Pattern = Adapter->uiHiPriorityPattern;

	Adapter->bPatternWritten = TRUE;
...
void InterfaceHandleShutdownModeWakeup(PMINI_ADAPTER Adapter)
{
    ...
    else
	{
        ...
        if(Adapter->chip_id>=BCS350)
        {
            ...
            Status = wrmalt (Adapter, DEVICE_INT_OUT_EP_REG1, &uiRegVal, sizeof(uiRegVal));
            if(Status)
            {
                BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0, "wrm failed while clearing H/W Abort Reg1");
                return;
