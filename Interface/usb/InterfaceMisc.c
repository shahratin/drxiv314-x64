            ...
            INT len)
{
	...
	if(retval < 0)
	{	
			BCM_DEBUG_PRINT(psIntfAdapter->psAdapter,DBG_TYPE_OTHERS, RDM, DBG_LVL_ALL, "RDM failed status :%d, retires :%d %d", retval,usRetries, addr);
            ...
            INT len)
{
	...
	if(retval < 0)
	{	
		BCM_DEBUG_PRINT(psIntfAdapter->psAdapter,DBG_TYPE_OTHERS, WRM, DBG_LVL_ALL, "WRM failed status :%d, retires :%d  addr: %x\n", retval, usRetries, addr);
		psIntfAdapter->psAdapter->DeviceAccess = FALSE ;
		return retval;
	}
	else
...
{   
    ...
    B_UINT32 CurrentTime = 0;
    B_UINT32 ulResetPattern[] = {HANDSHAKE_PATTERN1,HANDSHAKE_PATTERN2};
    UINT  uiRegRead = 0;
    int   status = 0;
    static B_UINT32 uiHandShakeTime = 0;
