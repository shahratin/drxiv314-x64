...
BOOLEAN IsReqGpioIsLedInNVM(PMINI_ADAPTER Adapter, UINT GpSpios,BOOLEAN bgpios)
{
	INT Status = 1;
    if (bgpios == WIMAX_IDX)
    	Status = (Adapter->gpioBitMap & GpSpios) ^ GpSpios;
    
    else if (bgpios == WIMAX_SPIO_IDX) //spio
        Status = (Adapter->spioBitMap & GpSpios) ^ GpSpios;
...
INT LED_Blink(PMINI_ADAPTER Adapter, UINT GPIO_Num, UCHAR uiLedIndex, B_UINT32 timeout, INT num_of_time, LedEventInfo_t currdriverstate)
{
...
INT ScaleRateofTransfer(B_UINT32 rate)
{
	if(rate <= 3)
		return rate;
		...
		UCHAR uiTxLedIndex, UCHAR GPIO_Num_rx, UCHAR uiRxLedIndex, LedEventInfo_t currdriverstate)
{
	...
	num_of_time_tx= ScaleRateofTransfer((B_UINT32)rate_of_transfer_tx);
	num_of_time_rx= ScaleRateofTransfer((B_UINT32)rate_of_transfer_rx);
													...
													USHORT usParamLen )
{
	...
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, LED_DUMP_INFO, DBG_LVL_ALL,"LED Thread:ValidateDSDParamsChecksum: 0x%x 0x%X",ulParamOffset, usParamLen);
...
INT ValidateHWParmStructure(PMINI_ADAPTER Adapter, B_UINT32 ulHwParamOffset)
{
...
int ReadLEDInformationFromEEPROM(PMINI_ADAPTER Adapter, UCHAR GPIO_Array[])
{
	...
	B_UINT32  dwReadValue 		= 0;
	USHORT usHwParamData 	= 0;
	UCHAR  ucIndex 			= 0;
    UCHAR  usEEPROMVersion[2]  = {0};
	UCHAR  ucGpSpIOInfo[32]    = {0};

	BeceemNVMRead(Adapter,(PUINT)&usEEPROMVersion,EEPROM_VERSION_OFFSET,2);

    BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, LED_DUMP_INFO, DBG_LVL_ALL,"usEEPROMVersion: Minor:0x%X Major:0x%x",usEEPROMVersion[0], usEEPROMVersion[1]);
	

	if((usEEPROMVersion[1]) < EEPROM_MAP5_MAJORVERSION)
	{
	...
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, LED_DUMP_INFO, DBG_LVL_ALL,"LED Thread: Start address of HW_PARAM structure = 0x%x",dwReadValue);
	...
	//
	if(dwReadValue > (Adapter->uiNVMDSDSize-DSD_START_OFFSET))
	{
	...
	*/
	...
	/* Read the GPIO values for 32 GPIOs from EEPROM and map the function
 	 ...
 	 */
	BeceemNVMRead(Adapter, (UINT *)ucGpSpIOInfo,dwReadValue,32);

	for(ucIndex = 0; ucIndex < 32; ucIndex++)
	 {
			
		 switch(ucGpSpIOInfo[ucIndex])
			{
				case RED_LED:
				{
        ...
        BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"GPIO's bit map correspond to LED :0x%X",Adapter->gpioBitMap);
        

		/*
		Sub GPIO_SECTION_START_OFFSET from the GPIO start offset within HW Param section.
		This will give the actual start HW Parameters start address.
		To read SPIO section, add SPIO offset further.
		*/
		dwReadValue -= GPIO_SECTION_START_OFFSET; // = get back to start address of hw param section.
		dwReadValue += SPIO_SECTION_START_OFFSET; // = SPIO start offset within HW Param section.

		/* Read the SPIO values for 32 SPIOs from EEPROM and map the function
		* number to SPIO pin number to SPIO_Array
		*/
		...
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, LED_DUMP_INFO, DBG_LVL_ALL,"SPIO's bit map correspond to LED :0x%X",Adapter->spioBitMap);
        BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"SPIO's bit map correspond to LED :0x%X",Adapter->spioBitMap);
...
INT LedGpioInit(PMINI_ADAPTER Adapter)
{
	UINT uiResetValue = 0;
	UINT uiIndex      = 0;
	INT status = STATUS_SUCCESS;
	/* Set all LED GPIO Mode to output mode */
	status = rdmaltWithLock(Adapter, GPIO_MODE_REGISTER, &uiResetValue, sizeof(uiResetValue));
	if(status < 0)
	{
		BCM_DEBUG_PRINT (Adapter, DBG_TYPE_PRINTK, 0, 0,"LED Thread: RDM Failed for GPIO Mode Read\n");
		return status;
	}
	...
	status = wrmaltWithLock(Adapter, GPIO_MODE_REGISTER, &uiResetValue, sizeof(uiResetValue));
	if(status < 0)
	{
		BCM_DEBUG_PRINT (Adapter, DBG_TYPE_PRINTK, 0, 0,"LED Thread: WRM Failed for GPIO Mode Write\n");
		return status;
	}
...
VOID LEDControlThread(PMINI_ADAPTER Adapter)
{
	...
	while(TRUE)
	{
		...
		{
			Status = LedGpioInit(Adapter);
			if(Status)
			{
				/* This will now invoke the led thread once another event is received */
				currdriverstate = Adapter->DriverState;
				continue;
			}
