...
//-----------------------------------------------------------------------------
// Procedure:	BeceemFlashBulkReadSHM
//
	...
	UINT uiNumBytes)
{
	...
	FLASH_CONTROL_AREA CAx   	= {0};
	UINT uiCaIndex				= 0;
	UINT uiBytesRead         	= 0;
	UINT  uiCurrentXferLen		= 0;
			
	

	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, NVM_RW, DBG_LVL_ALL,"BeceemFlashBulkReadSHM ====> \n");
	
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, NVM_RW, DBG_LVL_ALL, "uiOffset: 0x%x\n",uiOffset);
	

	while(uiBytesRead < uiNumBytes)
	{	
		...
		//
		// Get a Control Area to read the data from the SHM.
		//
		Status = GetControlArea(Adapter,&CAx,&uiCaIndex);

		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"GetControlArea returned an error:%d, line: %d\n",Status,__LINE__);			
			break;
		}

		//
		// Populate the CAx structure and then write in to the CA SHM area.
		//


		uiCurrentXferLen	= MIN(Adapter->uiSectorSize,(uiNumBytes-uiBytesRead));

		
		
		CAx.u32Command		= FLASH_READ;
		CAx.u32Handshake	= CA_VALID;
		CAx.u32DataAreaAddr = Adapter->sFlashShmInfo[uiCaIndex].u32DataAreaAddr;
		CAx.u32FlashAddr	= uiOffset + uiBytesRead;
		CAx.u32DataSize 	= uiCurrentXferLen;
			
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL,"uiOffset: 0x%x uiBytesRead: 0x%x CAx.u32FlashAddr: 0x%x\n",
			uiOffset,uiBytesRead,CAx.u32FlashAddr);

		
		Status = wrmalt(Adapter,
					Adapter->sFlashShmInfo[uiCaIndex].u32ControlAreaAddr,
					(PUINT)&CAx,
					sizeof(CAx));
				
		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"Could not write the CA: %d, %d\n",Status,__LINE__);
			break;
		}		
		

		
		Status = WaitCaStatus(Adapter, 
							Adapter->sFlashShmInfo[uiCaIndex].u32ControlAreaAddr,
							CA_READ_DONE);
		...
		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"WaitCaStatus failed: %d, %d\n",Status,__LINE__);
			break;
		}		
		else
		{		
			...
			//
			// We got a CA but handshake is CA_READ_DONE.
			// Fw has done with flash read and updated the SHM.
			// Now we have to go and read the SHM. After that this CA can be re-used.
			//
			Status = FlashReadSHM(Adapter,
						Adapter->sFlashShmInfo[uiCaIndex].u32DataAreaAddr,
						(PUCHAR)(((PUCHAR)pBuffer)+uiBytesRead),
						uiCurrentXferLen);

			if(Status != STATUS_SUCCESS)
			{
				BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"FlashReadSHM returned an error: %d, line: %d\n",Status, __LINE__);
				break;
			}

			
			uiBytesRead += uiCurrentXferLen;	
			//
			// Mark the CA as Free.
			//			
			CAx.u32Command		= 0;
			CAx.u32Handshake	= CA_FREE;
			CAx.u32DataAreaAddr = 0;
			CAx.u32DataOffset	= 0;
			CAx.u32FlashAddr	= 0;
			CAx.u32DataSize 	= 0;
	...
	CAx.u32Command		= END_OF_FLASH_ACCESS;
	CAx.u32Handshake	= CA_VALID;
	CAx.u32DataAreaAddr = 0;
	CAx.u32DataOffset	= 0;
	CAx.u32FlashAddr	= 0;
	...
	if(Status == STATUS_SUCCESS)
	{
		Status = AreAllTheCAsFree(Adapter);
	}											

	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, NVM_RW, DBG_LVL_ALL,"BeceemFlashBulkReadSHM <==== \n");		
	return Status;
} /* BeceemFlashBulkReadSHM() */
...
VOID BcmRestoreBlockProtectStatus(PMINI_ADAPTER Adapter,B_UINT32 ulWriteStatus)
{
	UINT value;

	if(Adapter->bFlashSHMEnabled == TRUE)
	{
		return;
	}
...
// Returns:
//		B_UINT32   - Status value before UnProtect.
//
//-----------------------------------------------------------------------------
B_UINT32 BcmFlashUnProtectBlock(PMINI_ADAPTER Adapter,UINT uiOffset, UINT uiLength)
{
	B_UINT32 ulStatus      = 0;
	B_UINT32 ulWriteStatus = 0;
	UINT value;
...
//-----------------------------------------------------------------------------
// Procedure:	BeceemFlashBulkWriteSHM
//
	...
	)
{
	...
	int Status 	= STATUS_SUCCESS;    
	FLASH_CONTROL_AREA CAx      = {0};
	eFlashCommands_t eCommand	= FLASH_WRITE;

	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS, NVM_RW, DBG_LVL_ALL, "BeceemFlashBulkWriteSHM ====> \n");
	
	
	uiOffsetFromSectStart 	= uiOffset & ~(Adapter->uiSectorSize - 1);
	uiCurrSectOffsetAddr	= uiOffset & (Adapter->uiSectorSize - 1);
	uiSectBoundary	  		= uiOffsetFromSectStart + Adapter->uiSectorSize;
...
//
	...
	{
		//      Number of sectors  = Last sector start Offset/sector size
	...
	if(IsFlash2x(Adapter) && (Adapter->bAllDSDWriteAllow == FALSE))
	{
		...
		while(uiTemp)
		{
			 ...
			 {
				BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL, "Sector Starting at offset <0X%X> is not writable",
											(uiOffsetFromSectStart + index * Adapter->uiSectorSize));
				Status = SECTOR_IS_NOT_WRITABLE;
				goto BeceemFlashBulkWriteSHM_EXIT;
			 }
			 uiTemp = uiTemp - 1;
			 index = index + 1 ;
		} 
	...
	if(Adapter->bWriteWOErase == TRUE)
	{
		eCommand = FLASH_WRITE_WO_ERASE;
	}
	else
	{
	...
	while(uiNumBytes)
	{
		...
		Status = GetControlArea(Adapter,&CAx,&index);

		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"GetControlArea failed with %d, Line:%d\n",Status, __LINE__);
			goto BeceemFlashBulkWriteSHM_EXIT;
		}
		
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL,"CA selected: %d\n",index);
		
		//
		// Initialize uiCurrentXferLen to min of sectorsize and Number of bytes remianing.
		//
			if((uiCurrSectOffsetAddr+uiNumBytes) > Adapter->uiSectorSize)
			{
				uiCurrentXferLen = Adapter->uiSectorSize - uiCurrSectOffsetAddr;
			}
			else
			{
				uiCurrentXferLen = MIN(Adapter->uiSectorSize,uiNumBytes);
			}
		
			
			if( TRUE == Adapter->bSigCorrupted &&
				1 == uiNumSectTobeRead		   &&
				eCommand == FLASH_WRITE 	   &&
				Adapter->uiNVMDSDSize > (uiCurrSectOffsetAddr+uiCurrentXferLen))
			{
				//
				// Issue a read if signature corruption is allowed. This is to restore the signature
				// along with the flash write so we will avoid one extra write to flash for signature alone.
				//		
				CAx.u32Command		= FLASH_READ;
				CAx.u32Handshake	= CA_VALID;
				CAx.u32DataAreaAddr = uiCurrSectOffsetAddr+uiCurrentXferLen+Adapter->sFlashShmInfo[index].u32DataAreaAddr;
				CAx.u32DataSize 	= Adapter->uiNVMDSDSize - (uiCurrSectOffsetAddr+uiCurrentXferLen);
				CAx.u32FlashAddr	= uiOffset+uiBytesWritten+uiCurrentXferLen;
		
				Status = wrmalt(Adapter,
							Adapter->sFlashShmInfo[index].u32ControlAreaAddr,
							(PUINT)&CAx,
							sizeof(CAx));
		
				if(Status != STATUS_SUCCESS)
				{
					BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"CA write failed with %d, Line:%d\n",Status, __LINE__);
					goto BeceemFlashBulkWriteSHM_EXIT;
				}
				...
				SaveHeaderIfPresentSHM(Adapter,
									   Adapter->sFlashShmInfo[index].u32DataAreaAddr,
									   uiOffset+uiBytesWritten);

				//
				// Since we are restoring the signature here, transfer length has been changed
				// to accomodate signature. Updating the length to include the additional bytes.
				//
				
				CAx.u32DataSize = Adapter->uiNVMDSDSize - uiCurrSectOffsetAddr;
			}
			else
			{
				CAx.u32DataSize = uiCurrentXferLen;
		...
		Status = FlashWriteSHM(Adapter,
					Adapter->sFlashShmInfo[index].u32DataAreaAddr+uiCurrSectOffsetAddr,
					((PUCHAR)pBuffer+uiBytesWritten),
					uiCurrentXferLen);

		if(Status != STATUS_SUCCESS)
		{
			
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0, "FlashWriteSHM failed with %d, Line:%d\n",Status, __LINE__);
			
			goto BeceemFlashBulkWriteSHM_EXIT;
		}

		CAx.u32Command 		= eCommand;
		CAx.u32Handshake	= CA_VALID;
		CAx.u32DataAreaAddr = Adapter->sFlashShmInfo[index].u32DataAreaAddr;
		CAx.u32DataOffset 	= uiCurrSectOffsetAddr+CAx.u32DataAreaAddr;
		CAx.u32FlashAddr 	= uiOffset+uiBytesWritten;
		...
		uiBytesWritten += uiCurrentXferLen;
		uiNumBytes 	   -= uiCurrentXferLen;
	...
	//
	// Wait till fw is done with flash write. Then write END_OF_FLASH_ACCESS to any of the free CA.
	//

	Status = AreAllTheCAsFree(Adapter);

	//
	// success means CAs are free. Else something wrong, return failure.
	//
	if(Status == STATUS_SUCCESS)
	{		
		CAx.u32Command		= END_OF_FLASH_ACCESS;
...
//
	...
	while(uiNumSectTobeRead)
	{
		...
		if(0 != (*Adapter->fpFlashBulkRead)(Adapter,
						(PUINT)pTempBuff,
		...
		{
			if(STATUS_SUCCESS == (*Adapter->fpFlashBulkRead)(Adapter,(PUINT)ucReadBk,uiOffsetFromSectStart+uiIndex,MAX_RW_SIZE))				
			{
...
//
	...
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"READING EE-PROMSIZE OFFSET FAILED... \n");
		return -1;
	}
...
//
	...
	if(uiEepromSize > 1024*1024)
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"Got EEPROM size is unexpected :%#X \n", uiEepromSize);
		return STATUS_SUCCESS;
	}
	...
	if ( pBuff == NULL )
	{
		...
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"Can not able to allocate memory of size : %#x(EpromSize)\n",uiEepromSize);
		return -1;
	}
	...
	{
		OsalMemFree(pBuff, 0);
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"NVM READ FAIL...\n");
		return -1;
	}
	...
	if( Adapter->chip_id >= BCS350 &&
	    FALSE == Adapter->bDisableFastFlashWrite)
	{
		if(IsFlash2x(Adapter))
		{
			OsalMemMove(pBuff,Adapter->psFlash2xCSInfo,MIN(BUFFER_1K,sizeof(FLASH2X_CS_INFO)));
					
			wrmaltWithLock(	Adapter,
							CONTROL_SECTION_SHM_ADDR,
							(PUINT)pBuff,
							MIN(BUFFER_1K,sizeof(FLASH2X_CS_INFO)));
		}
		else
		{
			OsalMemMove(pBuff,Adapter->psFlashCSInfo,MIN(BUFFER_1K,sizeof(FLASH_CS_INFO)))	;
				
			wrmaltWithLock( Adapter,
							CONTROL_SECTION_SHM_ADDR,
							(PUINT)pBuff,
							MIN(BUFFER_1K,sizeof(FLASH_CS_INFO)));
	...
	UINT uiNumBytes)
{
	...
	if(Adapter->eNVMType == NVM_FLASH)
	{
		...
		Status = (*Adapter->fpFlashBulkRead)(Adapter,
						pBuffer,
						uiOffset,
						uiNumBytes);
	...
	BOOLEAN bVerify)
{
	...
	if(Adapter->eNVMType == NVM_FLASH)
	{
		...
		else
		{
			...
			Status = (*Adapter->fpFlashBulkWrite)(Adapter,
						pBuffer,
						uiFlashOffset,
						uiNumBytes,
						bVerify);
...
//	
	...
	(*Adapter->fpFlashBulkRead)(Adapter,(PUINT)&sFlashCsInfo,Adapter->ulFlashControlSectionStart,sizeof(sFlashCsInfo));
...
//		Flash ID
//-----------------------------------------------------------------------------
B_UINT32 BcmReadFlashRDID(PMINI_ADAPTER Adapter)
{
	B_UINT32 ulRDID = 0;
	UINT value;
// 
...
VOID UpdateVendorInfo(PMINI_ADAPTER Adapter)
{
	...
	while(i < TOTAL_SECTIONS)
	{
		...
		case CONTROL_SECTION:
			if(UNINIT_PTR_IN_CS != Adapter->psFlash2xVendorInfo->VendorSection[i].OffsetFromZeroForSectionStart)
				Adapter->psFlash2xCSInfo->OffsetFromZeroForControlSectionStart = Adapter->psFlash2xCSInfo->OffsetFromZeroForControlSectionData = VENDOR_PTR_IN_CS;
			else
				Adapter->psFlash2xCSInfo->OffsetFromZeroForControlSectionStart = Adapter->psFlash2xCSInfo->OffsetFromZeroForControlSectionData = UNINIT_PTR_IN_CS;
			break;
...
INT BcmGetFlashCSInfo(PMINI_ADAPTER Adapter)
{
	...
	Adapter->fpFlashBulkRead  = BeceemFlashBulkRead;
	
	if(Adapter->bStatusWrite == TRUE)
	{
		Adapter->fpFlashBulkWrite = BeceemFlashBulkWriteStatus;
	}
	else
	{
		Adapter->fpFlashBulkWrite = BeceemFlashBulkWrite;
	}
	...
	if(!Adapter->bDDRInitDone)
	{
		if(Adapter->chip_id >= BCS350)
		{
			value = FLASH_CONTIGIOUS_START_ADDR_BCS350;
			wrmalt(Adapter, 0x0F00A080, &value, sizeof(value));
			value = FLASH_CONTIGIOUS_END_ADDR_BCS350;
			wrmalt(Adapter, 0x0F00A084, &value, sizeof(value));
		}
	...
	if(Adapter->chip_id >= BCS350)
	{
		rdmalt(Adapter, PAD_SELECT_REGISTER, &value, sizeof(value));
		value |= 0x1;
		wrmalt(Adapter, PAD_SELECT_REGISTER, &value, sizeof(value));

		value= 0x200;
		wrmalt(Adapter,FLASH_GPIO_CONFIG_REG,&value, sizeof(value));
	}
#endif

	if(STATUS_SUCCESS == vendorextndefineCS(Adapter))
	{
		Adapter->psFlashCSInfo->MagicNumber = htonl(FLASH_CONTROL_STRUCT_SIGNATURE);
		Adapter->psFlashCSInfo->FlashLayoutVersion = FLASH_2X_MAJOR_NUMBER;
		Adapter->uiVendorExtnFlag = TRUE;
		Adapter->psFlash2xVendorInfo->VendorSection[NO_SECTION_VAL].AccessFlags = FLASH2X_SECTION_PRESENT;
		Adapter->psFlash2xVendorInfo->VendorSection[NO_SECTION_VAL].OffsetFromZeroForSectionStart = 0;
	}
	else
	{
	// Reading first 8 Bytes to get the Flash Layout 
	// MagicNumber(4 bytes) +FlashLayoutMinorVersion(2 Bytes) +FlashLayoutMajorVersion(2 Bytes) 
	(*Adapter->fpFlashBulkRead)(Adapter,(PUINT)Adapter->psFlashCSInfo,Adapter->ulFlashControlSectionStart,8);
	...
	{	
		(*Adapter->fpFlashBulkRead)(Adapter,(PUINT)Adapter->psFlashCSInfo,Adapter->ulFlashControlSectionStart,sizeof(FLASH_CS_INFO));
...
NVM_TYPE BcmGetNvmType(PMINI_ADAPTER Adapter)
{
	...
	else if(Adapter->chip_id >= BCS350)
	{
		if(BcmGetEEPROMSize(Adapter))
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0,"NVM TYPE EEPROM - Fresh EEPROM with NO Signature\n");
			return NVM_EEPROM;
		}
	}
	...
	(*Adapter->fpFlashBulkRead)(Adapter,&uiData,0x0+Adapter->ulFlashCalStart,4);
	if(uiData == BECM)
	{
		return NVM_FLASH;
	}
	...
	UINT uiNumBytes)
{
	...
	Status = (*Adapter->fpFlashBulkRead)(Adapter, pBuffer,uiAbsoluteOffset,uiNumBytes);
	...
	UINT bVerify)
{
	...
	Status = (*Adapter->fpFlashBulkWrite)(Adapter, pBuffer,uiOffset,uiNumBytes,(BOOLEAN)bVerify);
...
INT GetFlashBaseAddr(PMINI_ADAPTER Adapter)
{
	...
	if(Adapter->chip_id >= BCS350)
	{
		return FLASH_CONTIGIOUS_START_ADDR_BCS350;
	}
...
/**
SaveHeaderIfPresentSHM :- This API is use to Protect the Header in case of Header Sector write
@Adapater :- Bcm Driver Private Data Structure
...
**/
INT SaveHeaderIfPresentSHM(PMINI_ADAPTER Adapter, B_UINT32 u32DataAddr, UINT uiOffset)
{
	...
	//In this case there will be only one  sector in which last one 1K will be header 	
	...
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"Warning:SaveHeaderIfPresentSHM: Entered in an unexpected case\n");
	}
	if(bHasHeader && Adapter->bSigCorrupted)
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL,"Signature Addr: 0x%lx u32DataAddr: 0x%x offsetToProtect:0x%x\n",
			(ULONG)(u32DataAddr + offsetToProtect + FIELD_OFFSET_IN_HEADER(PDSD_HEADER,DSDImageMagicNumber)),
			u32DataAddr,
			offsetToProtect);

		FlashReadSHM(Adapter,
			u32DataAddr + offsetToProtect + FIELD_OFFSET_IN_HEADER(PDSD_HEADER,DSDImageMagicNumber),
			(PUCHAR)&sig,
			sizeof(sig));
		...
		if((sig & 0xFF000000) != CORRUPTED_PATTERN)
		{
			...
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL,"Pattern: 0x%x\n",sig);
			Adapter->bSigCorrupted = FALSE;
			return STATUS_SUCCESS;
		}
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL, "corrupted sig is :%X", sig);

		sig = htonl(DSD_IMAGE_MAGIC_NUMBER);

		FlashWriteSHM(Adapter,
			u32DataAddr + offsetToProtect + FIELD_OFFSET_IN_HEADER(PDSD_HEADER,DSDImageMagicNumber),
			(PUCHAR)&sig,
			sizeof(sig));
...
INT SaveHeaderIfPresent(PMINI_ADAPTER Adapter, PUCHAR pBuff, UINT uiOffset)
{
	...
	if(bHasHeader && (Adapter->bHeaderChangeAllowed == FALSE))
	{
		...
		//Read header 
		(*Adapter->fpFlashBulkRead)(Adapter,(PUINT)pTempBuff,(uiSectAlignAddr + offsetToProtect),HeaderSizeToProtect);
										...
										)
{
  ...
  Status = bcmflash_raw_writenoerase((uiOffset/FLASH_PART_SIZE),(uiOffset % FLASH_PART_SIZE), pcBuff,uiNumBytes);
	...
	if(Adapter->bFlashSHMEnabled == TRUE)
	{
		Adapter->bWriteWOErase = TRUE;
		
		(*Adapter->fpFlashBulkWrite)(Adapter,
					pBuff,
					uiOffset,
					uiNumBytes,
					FALSE);

		Adapter->bWriteWOErase = FALSE;
	}
	else
	{
		BcmDoChipSelect(Adapter,uiOffset);
...
//-----------------------------------------------------------------------------
// Procedure:	GetFlashShmInfo
//
// Description: This routine assigns the input buffer to flash shm structure. 
//
// Arguments:
//		Adapter    - ptr to Adapter object instance
//     	pvBuffer   - Buffer which contain the Flash SHM info.
//
// Returns:
//		<VOID>
//-----------------------------------------------------------------------------
VOID GetFlashShmInfo(PMINI_ADAPTER Adapter, PVOID pvBuffer)
{
	FLASH_SHM_MSG sShmMsg = {0};
	FLASH_CONTROL_AREA CAx = {0};
	...
	if(Adapter->chip_id < BCS350)
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"Fast Flash access through SHM is not supported on this chipset\n");	
		return;
	}

	if(TRUE == Adapter->bDisableFastFlashWrite)
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"We cant process this request as we are asked to turn off this\n");
		return;
	}

	OsalMemMove(&sShmMsg,pvBuffer,sizeof(FLASH_SHM_MSG));


	Adapter->sFlashShmInfo[0].u32ControlAreaAddr = ntohl(sShmMsg.u32ControlArea1Addr);
	Adapter->sFlashShmInfo[0].u32DataAreaAddr    = ntohl(sShmMsg.u32DataArea1Addr);
	Adapter->sFlashShmInfo[0].u32DataAreaLength	 = ntohl(sShmMsg.u32DataArea1Length);

	
	Adapter->sFlashShmInfo[1].u32ControlAreaAddr = ntohl(sShmMsg.u32ControlArea2Addr);
	Adapter->sFlashShmInfo[1].u32DataAreaAddr    = ntohl(sShmMsg.u32DataArea2Addr);
	Adapter->sFlashShmInfo[1].u32DataAreaLength	 = ntohl(sShmMsg.u32DataArea2Length);

	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32ControlArea1Addr  : 0x%x\n",Adapter->sFlashShmInfo[0].u32ControlAreaAddr);   	
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32DataArea1Addr     : 0x%x\n",Adapter->sFlashShmInfo[0].u32DataAreaAddr);
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32DataArea1Length   : 0x%x\n",Adapter->sFlashShmInfo[0].u32DataAreaLength);   
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32ControlArea2Addr  : 0x%x\n",Adapter->sFlashShmInfo[1].u32ControlAreaAddr);
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32DataArea2Addr     : 0x%x\n",Adapter->sFlashShmInfo[1].u32DataAreaAddr);	
	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL, "u32DataArea2Length   : 0x%x\n",Adapter->sFlashShmInfo[1].u32DataAreaLength);
	
	if(Adapter->sFlashShmInfo[0].u32ControlAreaAddr)
	{
		...
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT ,MP_INIT,DBG_LVL_ALL,"CA1:u32ControlAreaInUse		 : 0x%x\n",CAx.u32ControlAreaInUse);
		
		if(CAx.u32ControlAreaInUse)
		{
			Adapter->uiCAcount++;
		}
		...
		//
		// Look whether second CA is available only if first one was available and valid.
		// If first CA itself is invalid no point in checking for second one.
		//
		
		if( Adapter->uiCAcount && 
			Adapter->sFlashShmInfo[1].u32ControlAreaAddr)
		{
			rdmalt(Adapter,
					Adapter->sFlashShmInfo[1].u32ControlAreaAddr,
			...
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"CA2:u32ControlAreaInUse 	 : 0x%x\n",CAx.u32ControlAreaInUse);
	...
	if(Adapter->uiCAcount)
	{
		...
		if ( (down_trylock(&Adapter->NVMRdmWrmLock)) == 0 )
		{
		...
		{
			Adapter->bUseFlashShmPtr = TRUE;		
		}
	}
	else
	{
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"Got a message from fw to select SHM based flash access but none of the CA are valid\n");
	}
...
//-----------------------------------------------------------------------------
// Procedure:	FlashReadSHM
//
// Description: Reads the data from SHM area.
//
// Arguments:
//		Adapter        - ptr to Adapter object instance
//     	uiDataAddr   - SHM Area address from where the data to be read .
//		pBuffer         - buffer for the data.
//           uiNumBytes  - num bytes to read.
//
// Returns:
//		<STATUS_CODE>
//-----------------------------------------------------------------------------
	...
	UINT uiNumBytes)
{
	...
	while(uiNumBytes)
	{
		...
		uiXferSize = MIN(MAX_XFER_SIZE,uiNumBytes);

		Status = rdm(Adapter,uiDataAddr+uiBytesRead,pBuffer+uiBytesRead,uiXferSize);



		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_OTHERS,NVM_RW,DBG_LVL_ALL,"FlashReadSHM:read fail during SHM read with %d, Line:%d\n",Status, __LINE__);
			break;
		}
		
		uiBytesRead += uiXferSize;
		uiNumBytes	-= uiXferSize;
	}
...
//-----------------------------------------------------------------------------
// Procedure:	FlashWriteSHM
//
// Description: Writes the data to SHM area.
//
// Arguments:
//		Adapter        - ptr to Adapter object instance
//     	uiDataAddr   - SHM Area address where the data to be written .
//		pBuffer         - Data to be written.
//           uiNumBytes  - num bytes to write.
//
// Returns:
//		<STATUS_CODE>
//-----------------------------------------------------------------------------
	...
	UINT uiNumBytes)
{
	...
	while(uiNumBytes)
	{
		...
		Status = wrm(Adapter,uiDataAddr+uiBytesWritten,(pBuffer+uiBytesWritten),uiXferSize);

		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"FlashWriteSHM:Write fail during SHM write with %d, Line:%d\n",Status, __LINE__);
			break;
		}
		
		uiBytesWritten += uiXferSize;
		uiNumBytes	   -= uiXferSize;
	}
...
//-----------------------------------------------------------------------------
// Procedure:	GetControlArea
//
// Description: checks if any Control Area is free. if free, selects and returns that.
//                   in case of read, it selects and returns the CA even in case of
//                   the handshake is CA_READ_DONE.
//
// Arguments:
//		Adapter        - ptr to Adapter object instance
//     	pCAx   	     - Control Area structure.
//		puiIndex       - Index to find out which Control Area is selected.
//
// Returns:
//		<STATUS_CODE>
//-----------------------------------------------------------------------------
...
int GetControlArea(
	PMINI_ADAPTER Adapter,
	PFLASH_CONTROL_AREA pCAx,
	PUINT puiIndex)
{
	
	UINT  uiCounter           	= 0;
	B_UINT32 ulCurrentTime    	= 0;
	B_UINT32 ulTimeSinceLastTry	= 0;
	int Status 	= STATUS_SUCCESS;      
	
	ulTimeSinceLastTry = jiffies;

	//
	// Loop through the Control Area in round robin fashion, and see if any of them is free.
	// Selects the free one by storing the index of that control section to puiIndex.
	// In case of FLASH_READ, we should select the ones with CA_READ_DONE status as well.
	//
	while(TRUE)
	{
		if(Adapter->device_removed )
		...
		Status = rdmalt(Adapter, 
								Adapter->sFlashShmInfo[(uiCounter%Adapter->uiCAcount)].u32ControlAreaAddr,
								(PUINT)pCAx,
								sizeof(FLASH_CONTROL_AREA));

		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"GetControlArea:read fail during CA read with %d, Line:%d\n",Status, __LINE__);
			break;
		}
		
		if(pCAx->u32ControlAreaInUse && pCAx->u32Handshake == CA_FREE)
		{
			//
			// Found a FREE CA.
			//
			*puiIndex = (uiCounter%Adapter->uiCAcount);
			break;
		}

		ulCurrentTime = jiffies;

		if((ulCurrentTime - ulTimeSinceLastTry) > SHM_ACCESS_TIMEOUT)
		{
			//
			// We dont have any response from the fw for the past SHM_ACCESS_TIMEOUT seconds.
			// Lets break out and return with a failure.
			//
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"GetControlArea:Timeout during CA read with %d, Line:%d\n",Status, __LINE__);
...
//-----------------------------------------------------------------------------
// Procedure:	AreAllTheCAsFree
//
// Description: Checks if all the CAs are free or not. 
//
// Arguments:
//		Adapter        - ptr to Adapter object instance
//
	...
	PMINI_ADAPTER Adapter )
{
	...
	//
	// Loop through all the Control Area available one by one
	// and see if they are free. 
	//

	while(uiIndex < Adapter->uiCAcount)
	{
		if(Adapter->device_removed )
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK, 0, 0, "Device has been removed");
		...
		Status = rdmalt(Adapter,
								Adapter->sFlashShmInfo[uiIndex].u32ControlAreaAddr,
								(PUINT)&CAx,
								sizeof(CAx));			

		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0, "AreAllTheCAsFree:read fail during CA read with %d, Line:%d\n",Status, __LINE__);
			break;
		}

		if(CAx.u32ControlAreaInUse == FALSE ||
		  (CAx.u32ControlAreaInUse && CAx.u32Handshake == CA_FREE))
		{
			uiIndex++;
			ulTimeSinceLastTry = jiffies;
		}
		...
		if((ulCurrentTime - ulTimeSinceLastTry) > SHM_ACCESS_TIMEOUT)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0, "AreAllTheCAsFree:Timeout during CA read with %d, Line:%d\n",Status, __LINE__);
			Status = STATUS_FAILURE;
			break;
		}
...
//-----------------------------------------------------------------------------
// Procedure:	WaitCaStatus
//
// Description: Checks if the specific CA has become 'eHandshake' state.
//
// Arguments:
//		Adapter        - ptr to Adapter object instance
//		u32CaAddr   - Addr of CA.
//		eHandShake - Handshake value to be checked.
//
// Returns:
//		<STATUS_CODE>
//-----------------------------------------------------------------------------
	...
	)
{
	...
	while(TRUE)
	{
		...
		if(Status != STATUS_SUCCESS)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0,"u32CaAddr:read fail during CA read with %d, Line:%d\n",Status, __LINE__);
			break;
		}

		if(CAx.u32ControlAreaInUse == FALSE ||
		  (CAx.u32ControlAreaInUse && CAx.u32Handshake == eHandshake))
		{
			break;
		}
		...
		if((ulCurrentTime - ulTimeSinceLastTry) > SHM_ACCESS_TIMEOUT)
		{
			BCM_DEBUG_PRINT(Adapter,DBG_TYPE_PRINTK ,0,0, "u32CaAddr:Timeout during CA read with %d, Line:%d\n",Status, __LINE__);
			Status = STATUS_FAILURE;
			break;
		}
	}
