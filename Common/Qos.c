...
BOOLEAN MatchSrcIpAddress(S_CLASSIFIER_RULE *pstClassifierRule,B_UINT32 ulSrcIP);
...
BOOLEAN MatchDestIpAddress(S_CLASSIFIER_RULE *pstClassifierRule,B_UINT32 ulDestIP);
...
//		SF_MAX_ALLOWED_PACKETS_TO_BACKUP)
	{
		...
		if((Adapter->PackInfo[iIndex].uiCurrentPacketsOnHost < SF_MAX_ALLOWED_PACKETS_TO_BACKUP) && 
			((B_UINT32)(1000*(jiffies - *((B_UINT32 *)(PacketToDrop->cb)+(B_UINT32)SKB_CB_LATENCY_OFFSET))/(B_UINT32)HZ) <= Adapter->PackInfo[iIndex].uiMaxLatency))
			break;
...
USHORT ClassifyPacket(PMINI_ADAPTER Adapter,struct sk_buff* skb)
{
	...
	*((UINT32*) (skb->cb) +SKB_CB_TCPACK_OFFSET ) = 0;
	...
	for(uiLoopIndex = MAX_CLASSIFIERS - 1; uiLoopIndex >= 0; uiLoopIndex--)
	{
		...
		do
		{
			...
			if(Adapter->PackInfo[uiSfIndex].bEthCSSupport)
			{
				...
				BCM_DEBUG_PRINT(Adapter, DBG_TYPE_TX, IPV4_DBG, DBG_LVL_ALL,  "Performing ETH CS Classification on Classifier Rule ID : %x Service Flow ID : %x\n",pstClassifierRule->uiClassifierRuleIndex,Adapter->PackInfo[uiSfIndex].ulSFID);
	...
	{			
		BCM_DEBUG_PRINT(Adapter,DBG_TYPE_TX, IPV4_DBG, DBG_LVL_ALL, "CF id : %d, SF ID is =%u",pstClassifierRule->uiClassifierRuleIndex, pstClassifierRule->ulSFID);	
